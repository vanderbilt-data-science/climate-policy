...
def maps_qa(api_key, user_input, extra_context, plan_list, state_abbr, epa_region):
    ...
    individual, combined, and EPA region vector stores.

    Aggregates retrievals into exactly four documents:
      1. Aggregated results from individual plan vector stores.
      2. A single result from the combined summary vector store.
      3. Aggregated results from multiple EPA region city vector stores.
      4. Extra context provided.

    Args:
        api_key (str): OpenAI API key.
        user_input (str): The user's question.
        extra_context (str): Additional context to include.
        plan_list (list): List of plan names.
        state_abbr (str): State abbreviation used to format plan names.
        epa_region (str): EPA region identifier.
    ...
    try:
        ...
        if combined_retrieved_chunks:
            combined_doc = combined_retrieved_chunks[0]
        else:
            combined_doc = Document(page_content="")
    all_documents.append(combined_doc)

    # 3. Retrieve from EPA region vector stores (aggregate from multiple cities)
    epa_region_dir = f"Combined_By_Region_Vectorstores/Region_{epa_region}"
    epa_texts = []
    if os.path.exists(epa_region_dir):
        for fname in os.listdir(epa_region_dir):
            # Process only directories that follow the naming convention for summary vectorstores.
            if not fname.endswith("_Summary_vectorstore"):
                continue
            store_path = os.path.join(epa_region_dir, fname)
            try:
                embedding_model = OpenAIEmbeddings(model="text-embedding-3-large")
                vector_store = FAISS.load_local(store_path, embedding_model, allow_dangerous_deserialization=True)
            except Exception as e:
                st.error(f"Error loading EPA region vector store '{fname}': {e}")
                continue
            retriever = vector_store.as_retriever(search_kwargs={"k": 1})
            retrieved_chunks = retriever.invoke(user_input)
            if retrieved_chunks:
                epa_texts.append(retrieved_chunks[0].page_content)
    else:
        st.error(f"EPA region directory not found: {epa_region_dir}")
    
    if epa_texts:
        aggregated_epa_text = "\n".join(epa_texts)
    else:
        aggregated_epa_text = ""
    epa_region_doc = Document(page_content=aggregated_epa_text)
    all_documents.append(epa_region_doc)
