...
from create_vector_store import create_and_save_vector_store, create_and_save_individual_vector_stores, list_vector_store_documents
...
# Create tabs
tab1, tab2, tab3, tab4 = st.tabs(["Summary Generation", "Multi-Plan QA (Shared Vectorstore)", "Multi-Plan QA (Multi-Vectorstore)", "Multi-Plan Comparison QA"])
...
# Fourth tab: One-to-Many
with tab4:
    # Function to load the vector store for a given document
    def load_vector_store(doc_name):
        vectorstore_path = f"Individual_Vectorstores/{doc_name}_vectorstore"
        embeddings = OpenAIEmbeddings(model="text-embedding-3-large")
        return FAISS.load_local(vectorstore_path, embeddings, allow_dangerous_deserialization=True)
    
    # Load list of documents from the vector store directory
    documents = list_vector_store_documents()


    st.header("Plan Comparison Tool")

    # Option to upload a new plan or select from a list
    upload_option = st.radio("Choose a focus plan:", ("Upload a new plan", "Select from existing plans"))

    if upload_option == "Upload a new plan":
        focus_uploaded_file = st.file_uploader("Upload a Climate Action Plan to compare", type="pdf")
        focus_city_name = st.text_input("Enter the city name for the uploaded plan:")
    ...
    # Primary document selection
        st.write("Choose the primary document for comparison:")
        primary_doc = st.selectbox("Select Primary Document", documents, help="Choose the main document for comparison")
    
    # secondary document selection
        st.write("Choose one or more documents to compare with primary document:")
        comparison_docs = st.multiselect("Select Documents to Compare",
                                         options =[doc for doc in documents if doc != primary_doc],
                                     default=[],
                                     help="Choose one or more documents to compare with primary document")
        #Prompt for comparison
        comparison_prompt_path = "comparison_prompt.md"
        try:
            with open(comparison_prompt_path, 'r') as file:
                comparison_prompt = file.read()
        except FileNotFoundError:
            comparison_prompt = None
            st.warning("Comparison prompt file not found")


    # Run comparison if selections are made and prompt is available
    if st.button("Compare") and primary_doc and comparison_docs and comparison_prompt:
        display_placeholder = st.empty()
        primary_vector_store = load_vector_store(primary_doc).as_retriever(search_kwargs={"k": 10})

    # Load and retrieve comparison documents
        for doc in comparison_docs:
            comparison_vector_store = load_vector_store(doc).as_retriever(search_kwargs={"k": 10})

        # Retrieve chunks from both primary and comparison document
            primary_chunks = primary_vector_store.invoke(comparison_prompt)
            comparison_chunks = comparison_vector_store.invoke(comparison_prompt)

        # Display the comparison results
            display_placeholder.markdown(
                f"**Comparison with {doc}:**\n"
                f"Primary Document Chunks: {primary_chunks}\n"
                f"Comparison Document Chunks: {comparison_chunks}"
            )

    #placeholder for customizing questions
        
    #input_text = st.text_input("Ask a comparative question:")

  #  if input_text and api_key and primary_file and comparison_files:
  #      display_placeholder = st.empty()
  #      process_one_to_many_query(api_key, primary_doc, comparison_docs, comparison_prompt, display_placeholder)
  #  else:
  #      st.info("Please select the primary and comparison document")
